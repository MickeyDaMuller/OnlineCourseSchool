...
$hearus = $conn->real_escape_string(trim($_POST['hearus']));
$areuborn = $conn->real_escape_string(trim($_POST['bornagain']));
$woulduborn = $conn->real_escape_string(trim($_POST['woulduborn']));
$areuholy = $conn->real_escape_string(trim($_POST['holy']));
$woulduholy = $conn->real_escape_string(trim($_POST['woulduholy']));
...
if (!empty($_POST['firstname']) && !empty($_POST['surname'])&& !empty($_POST['username'])&& !empty($_POST['email'])&& !empty($_POST['password'])&& !empty($_POST['gender'])&& !empty($_POST['age'])&& !empty($_POST['country'])&& !empty($_POST['nationality'])&& !empty($_POST['phone'])&& !empty($_POST['hearus'])&& !empty($_POST['bornagain'])&& !empty($_POST['holy'])&& !empty($file)) {
  ...
  $sql = "SELECT email FROM users WHERE email = '$email'";
      ...
      $count = mysqli_num_rows($result);
        ...
        if($count == 0){
          ...
          $query   = "INSERT into users (firstname,surname,username,email,password,gender,age,myimage,country,nationality,phone,hearus,areuborn,woulduborn,areuholy,woulduholy,com_code,created) VALUES('" . $firstname . "','" . $surname . "','" . $username . "','" . $email . "','" . $password . "','" . $gender . "','" . $age . "','" . $file . "','" . $country . "','" . $nationality . "','" . $phone . "','" . $hearus . "','" . $areuborn . "','" . $woulduborn . "','" . $areuholy . "','" . $woulduholy . "','" . $com_code . "','" . $time_date . "')";
...
//echo "$success";
 ...
 elseif (empty($_POST['firstname']) && empty($_POST['surname'])&& empty($_POST['username'])&& empty($_POST['email'])&& empty($_POST['password'])&& empty($_POST['gender'])&& empty($_POST['age'])&& empty($_POST['country'])&& empty($_POST['nationality'])&& empty($_POST['phone'])&& empty($_POST['hearus'])&& !empty($_POST['bornagain'])&& !empty($_POST['holy'])&& !empty($file)) {
...
<a href='http://www.globalyouthleadersforum.org/gylf_academy/confirm.php?passkey=$com_code'>http://www.globalyouthleadersforum.org/gylf_academy/confirm.php?passkey=$com_code</a><br><br><br>
...
<label><font color="#008bc4">Profile Image (maximum image size of 500KB):</font></label>
 ...
 <input type="file" name="myimage" id="myimage" accept="image/*" onchange="checkPhoto(this)" style="border:2px solid #008bc4; padding: 20px; padding-left: 25px; padding-right: 23px; text-align: center;" required>
