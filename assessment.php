...
$sql = "SELECT id,essay FROM gylf_academy_module1_essay ORDER BY created DESC";
...
$sql = "SELECT * FROM gylf_academy_module1 WHERE ref_id = $ref_id order by questionnumber asc";
  $res = mysqli_query($conn, $sql);
...
$query = "SELECT answer FROM gylf_academy_module1 WHERE ref_id = $ref order by questionnumber asc";
...
$sql = "INSERT INTO academy_scores (user_id, essay_id, scores, created) VALUES ('$user_id', '$ref', '$total', '$time_date')";
...
$mail->Subject = "GYLF Academy Module $ref Score";
$mail->Body    = "
...
<?php
if ($user_id==$id && $essayid==$essay_id && !empty($scores)) {
  echo '<script>alert("You have answered this module.")</script>';
  echo "<center><b>Next module coming soon!!!</b></center>";
}
else{

echo "<form action = 'assessment.php' method = 'post' id='assessmentform' name='assessmentform'>";
...
echo "<input type='radio' name='itemtype".$i."' value='d'>".$d."</input><br />";
echo "<input type='radio' name='itemtype".$i."' value='e'>".$e."</input><br /><br /><br /><br />";
